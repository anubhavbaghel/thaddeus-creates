...
function HomePage() {
  return (
    <main className="overflow-hidden">
      ...
      <section className="mx-auto grid max-w-7xl gap-14 px-5 py-16 sm:px-8 sm:py-24 lg:grid-cols-2 lg:gap-24 lg:px-12"><div><p className="section-kicker">Make it unmistakably theirs</p><h2 className="mt-3 max-w-[12ch] font-display text-4xl font-light leading-[1.02] sm:text-6xl">Your story is the finishing touch.</h2><p className="mt-6 max-w-lg leading-7 text-muted-foreground">Bring the photo, the name, the date or even a half-formed idea. Each detail is chosen with you, then made slowly by hand.</p><div className="mt-8 flex flex-wrap gap-2.5">{["Names & dates", "Your photos", "Colour stories", "Flowers & finishes", "Gift wrapping"].map((item) => <span key={item} className="rounded-md border border-border bg-card px-4 py-2 text-sm font-semibold">{item}</span>)}</div></div><div><p className="section-kicker">From hello to handmade</p><ol className="mt-5 border-t border-border">{[["01", "Share your idea", "Tell us who it is for, the occasion and the little details you love."], ["02", "Shape it together", "We align the colours, photos, names and finishing touches with you."], ["03", "Made just for you", "Your piece is crafted by hand and prepared to become part of the moment."]].map(([number, title, copy]) => <li key={number} className="grid grid-cols-[3.5rem_1fr] gap-4 border-b border-border py-6"><span className="pt-1 font-display text-4xl font-light leading-none text-primary">{number}</span><div><h3 className="font-display text-xl font-medium">{title}</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">{copy}</p></div></li>)}</ol></div></section>
